...
import { mapWithConcurrency, pokeFetch } from "./fetcher";

const MAX_ID = 1025;

// Per-detail-page snapshot. damageRelations is stored once globally (it is the
// same for every Pokémon) instead of duplicated into all 1025 files.
export type DetailSnapshot = {
  full: Omit<PokemonFull, "damageRelations">;
  evolution: EvolutionBundle;
  abilityDetail: Record<string, AbilityDetail>;
  moveDetail: Record<string, MoveDetail>;
};

// Base species (id <= MAX_ID) use locally-downloaded webp (see scripts/build-sprites.ts).
// Alt forms (id > MAX_ID) stay on the GitHub CDN.
const REMOTE_ART = (id: number) =>
  `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${id}.png`;
const REMOTE_SPRITE = (id: number) =>
  `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`;
const ART = (id: number) =>
  id <= MAX_ID ? `/sprites/art/${id}.webp` : REMOTE_ART(id);
const SPRITE = (id: number) =>
  id <= MAX_ID ? `/sprites/sprite/${id}.webp` : REMOTE_SPRITE(id);
...
function pickLocalizedNames(species: Pokedex.PokemonSpecies): LocalizedName[] {
  ...
  for (const entry of species.names) byLang.set(entry.language.name, entry.name);
...
function buildFormAbilities(pkmn: Pokedex.Pokemon): Ability[] {
  ...
  for (const a of ordered) {
    ...
    seen.add(slug);
    out.push({ slug, name: titleCase(slug), isHidden: !!a.is_hidden });
  }
  return out;
}
...
async function buildForm(
  ...
  try {
    const pkmn =
      cached ?? (await pokeFetch<Pokedex.Pokemon>(`/pokemon/${variety.pokemon.name}/`));
...
function buildMovepool(pkmn: Pokedex.Pokemon, vg: string | null): MoveEntry[] {
  ...
  for (const m of pkmn.moves) {
    const entries = m.version_group_details.filter((d) => d.version_group.name === vg);
...
function pickAbilityShortEffect(a: Pokedex.Ability): string {
  return cleanText(a.effect_entries.find((e) => e.language.name === "en")?.short_effect ?? "");
}
function pickAbilityEffect(a: Pokedex.Ability): string {
  return cleanText(a.effect_entries.find((e) => e.language.name === "en")?.effect ?? "");
}
...
async function fetchAbilityDetails(slugs: string[]): Promise<Record<string, AbilityDetail>> {
  ...
  const arr = await Promise.all(
    slugs.map((s) => pokeFetch<Pokedex.Ability>(`/ability/${s}/`)),
  );
...
async function fetchMoveDetails(slugs: string[]): Promise<Record<string, MoveDetail>> {
  if (slugs.length === 0) return {};
  const arr = await Promise.all(
    slugs.map((s) => pokeFetch<Pokedex.Move>(`/move/${s}/`)),
  );
...
export async function getDamageRelations(): Promise<DamageRelations> {
  const arr = await Promise.all(
    ALL_TYPES.map((t) => pokeFetch<Pokedex.Type>(`/type/${t}/`)),
  );
...
export async function getAllPokemonLite(): Promise<PokemonLite[]> {
  const typeResults = await Promise.all(
    TYPE_NAMES.map((t) => pokeFetch<Pokedex.Type>(`/type/${t}/`)),
  );
  ...
  const genResults = await Promise.all(
    genNames.map((g) => pokeFetch<Pokedex.Generation>(`/generation/${g}/`)),
  );
  ...
  const fullList = await pokeFetch<Pokedex.NamedAPIResourceList>(
  ...
  const baseSlugs = baseEntries.map((e) => e.name).sort((a, b) => b.length - a.length);

  const tagsBySpeciesId = new Map<number, Set<FormCategory>>();
  const slugToId = new Map<string, number>(baseEntries.map((e) => [e.name, e.id]));
  ...
  for (const form of formEntries) {
    ...
    const suffix =
      form.name === speciesSlug ? "" : form.name.slice(speciesSlug.length + 1);
    const cat = detectFormCategory(suffix);
    if (!cat) continue;
  ...
  const speciesList = await mapWithConcurrency(speciesIds, 24, (id) =>
    pokeFetch<Pokedex.PokemonSpecies>(`/pokemon-species/${id}/`),
  );
  const romajiBySpeciesId = new Map<number, string | null>();
  for (const sp of speciesList) romajiBySpeciesId.set(sp.id, pickRomaji(sp));
...
// One merged fetch per detail page: species + all forms + movepool + move
// details + ability details + evolution chain. Everything the detail view
// needs, resolved at build time. (damageRelations is global, fetched once.)
export async function getDetailSnapshot(
  id: number,
  liteById: Map<number, PokemonLite>,
): Promise<DetailSnapshot> {
  const [pkmn, species] = await Promise.all([
    pokeFetch<Pokedex.Pokemon>(`/pokemon/${id}/`),
    pokeFetch<Pokedex.PokemonSpecies>(`/pokemon-species/${id}/`),
  ]);
  ...
  // Ability + move detail (deduped across all forms).
  const abilitySlugs = Array.from(
    new Set(formBundles.flatMap((b) => b.form.abilities.map((a) => a.slug))),
  );
  ...
  // Evolution chain + lite info for each member.
  const chainId = urlToId(species.evolution_chain.url);
  const chain = await pokeFetch<Pokedex.EvolutionChain>(`/evolution-chain/${chainId}/`);
  const evolution = buildEvoTree(chain.chain as unknown as EvoLink);
  const evoIds: number[] = [];
  ...
  for (const eid of evoIds) {
    const p = liteById.get(eid);
    if (p) evoLites[p.id] = p;
  }

  const full: Omit<PokemonFull, "damageRelations"> = {
    id: pkmn.id,
