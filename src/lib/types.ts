...
// Build-resolved evolution data (replaces the streamed PokemonEvolutionBundle).
export type EvolutionBundle = {
  evolution: EvolutionNode;
  babyTriggerItem: string | null;
  evoLites: Record<number, PokemonLite>;
};

// Shape of each src/data/pokemon/<id>.json snapshot file. damageRelations is
// stored once in src/data/type-relations.json (shared) and merged back into
// `full` at page-build time.
...
// Split snapshot: page-critical data is embedded in HTML (small); the heavy
// move/ability effect text is fetched client-side from a static JSON file.
